...
class Annotations(object):
    ...
    def __init__(self, document, read_only=False):
        ...
        self._parse_function_by_id_prefix = {
            ...
            'F': self._parse_textlevel_annotation, #F added by Sander Naert
    ...
    def get_textLevels(self):
        return [a for a in self if isinstance(a, TextLevelAnnotation)]
...
class TextBoundAnnotationWithText(TextBoundAnnotation):
 ...
 #Added by Sander Naert
class TextLevelAnnotation(IdedAnnotation):
    '''
    Annotation on text level.
    '''
    def __init__(self, id, type, ids, tail="", source_id=None):
        IdedAnnotation.__init__(self, id, type, tail, source_id=source_id)
        self.ids = ids
...
#Added by Sander Naert
class SimpleAnnotations(object):
    ...
    def __init__(self, annotations):
        self._document = annotations._document
        self.failed_lines = annotations.failed_lines
        self.externally_referenced_triggers = annotations.externally_referenced_triggers
        self._ann_by_id = annotations._ann_by_id
        self._lines = annotations._lines
        self._line_by_ann = annotations._line_by_ann
        self._read_only = annotations._read_only
        self._input_files = annotations._input_files
        self.ann_mtime = annotations.ann_mtime
        self.ann_ctime = annotations.ann_ctime
        self.folia = {"entities":[],"comments":[],"relations":[],"attributes":[],"tokens":{}}
        self.issues = []
        self.validated = False
        try:
            if(annotations._document_text):
                self._document_text = annotations._document_text
        except:
            pass
    ...
    #Added by Sander Naert, there can only be 1
    def get_textLevels(self):
        return [a for a in self if isinstance(a, TextLevelAnnotation)]
    def get_document_text(self):
        return self._document_text
    ...
    def add_annotation(self, ann, read=False):
        ...
        try:
            self._ann_by_id[ann.id] = ann
            pre, num = annotation_id_prefix(ann.id), annotation_id_number(ann.id)
            self.max_id_num_by_prefix[pre] = max(num, self.max_id_num_by_prefix[pre])
        except AttributeError:
            # The annotation simply lacked an id which is fine
            pass
   ...
   #CHANGED by sander naert to speed up __exit__
    #safe_exit is the original, fast_exit is the new one.
    def fast_exit(self):
        if not self._read_only:
            assert len(self._input_files) == 1, 'more than one valid outfile'
            out_str = unicode(self)
            ...
            with file_lock(path_join(WORK_DIR,
                ...
                try:
                    with open_textfile(self._input_files[0], 'w') as tmp_file:
                        tmp_file.write(out_str)
                except:
                    Messager.error("Problem trying to write the changes "+str('error'))
                    raise

                from ApplicationScope import update_pickle
                update_pickle(self)
            return
